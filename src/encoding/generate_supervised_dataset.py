...
def generate_supevised_dataset(
    ...
    df = reviewed_data[["codigo", "id_pieza", "CODART_A3", "CAR3", "text_to_analyse", "processed_text_to_analyse", "Cod_error", "highest_score_error"]]
    clean_df = df[df["CAR3"] == 91]
    clean_df = clean_df[
        (~clean_df["Cod_error"].isna()) & (~clean_df["Cod_error"].isin(["na", "n", "nn", "309", " na"]))]
