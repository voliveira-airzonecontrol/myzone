...
class Corpus:
    ...
    def get_corpus(self) -> pd.DataFrame:
        return pd.concat([self.incidencias, self.faq, self.products_docs, self.catalogo])




    def prepare_incidencias(
            self,
            translation_date: str = "2024-05-09"
    ) -> 'Corpus':
        """
        ...
        incidencias = Incidencias().get_incidencias(limit_date=translation_date).incidencias

        self.incidencias = pd.DataFrame({"text_to_analyse": incidencias["text_to_analyse"]})
    ...
    def prepare_faq(self) -> 'Corpus':
        ...
        """
        faq = pd.read_csv(os.path.join(TRAINING_DATA_PATH, "FAQ.csv"), sep=';', header=None)
    ...
    def prepare_products_docs(self) -> 'Corpus':
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            results = list(tqdm(executor.map(self.__process_pdf, pdfs), total=len(pdfs)))
    ...
    def prepare_catalogo(self) -> 'Corpus':
        ...
        :return: Corpus object
        """
        self.catalogo = self.__extract_text_from_pdf(os.path.join(TRAINING_DATA_PATH, "catalogo.pdf"))
